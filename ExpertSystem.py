...
class ExpertSystem:
    def __init__(self, concepts):
        ...
        self.data1, self.data2 = data.get_initial_data()
        self.find_concepts(self.data1, self.data2)
        self.recomends = data.get_final_predicts(self.hard_concept)
        self.answer = self.get_answer(self.recomends)

    def find_concepts(self, data1, data2):
        self.hard_concept = []
        for i in range(10):
            ...
            for key, value in data2.items():
                for i in value:
                    if any(elem in self.hard_concept or elem in self.concepts for elem in i):
                        self.hard_concept.append(key)
